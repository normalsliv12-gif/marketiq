...
async function loadPredictions() {
    ...
    console.log('📅 Loading predictions for:', weekKey);
...
function updatePredictionTimer() {
    const timeObj = getTimeUntilWeekEnd();
    const formatted = formatTimeRemaining(timeObj);
    const el = document.getElementById('predTimeRemaining');
    if (el) el.textContent = formatted;
}

// Render prediction question cards
function renderPredictionQuestions() {
    const container = document.getElementById('predQuestionsContainer');
    
    container.innerHTML = PREDICTION_QUESTIONS.map(q => `
        <div class="pred-question-card">
            <div class="pred-q-header">
                <span class="pred-q-label">Question ${q.id.toUpperCase().replace('Q', '')}</span>
            ...
            <div class="pred-slider-wrap">
                <input type="range" class="pred-slider" id="slider_${q.id}" 
                       min="0" max="100" value="50" 
                       oninput="updatePredictionValue('${q.id}', this.value)">
                <div class="pred-value-display">
                    <span class="pred-value-num" id="value_${q.id}">50</span>
                    <span class="pred-value-label">%</span>
                </div>
            </div>
...
// Update prediction value from slider
function updatePredictionValue(questionId, value) {
    ...
    // Store answer
    predictionAnswers[questionId] = numValue;
    
    // Check if all answered
    const allAnswered = Object.values(predictionAnswers).every(v => v !== null);
    const submitBtn = document.getElementById('predSubmitBtn');
    if (submitBtn) {
        submitBtn.disabled = !allAnswered;
        submitBtn.textContent = allAnswered ? 'Submit Forecasts' : 'Submit Forecasts (Locked Until All Answered)';
    }
}
...
async function submitPredictions() {
    ...
    const allAnswered = Object.values(predictionAnswers).every(v => v !== null);
    if (!allAnswered) {
        showToast('Please answer all 3 questions', 'warning');
        return;
    }
    ...
    try {
        ...
        PREDICTION_QUESTIONS.forEach(q => {
            const prob = predictionAnswers[q.id];
            const predRef = db.collection('predictions').doc(weekKey)
                              .collection(q.id).doc(currentUser.username);
            batch.set(predRef, {
                probability: prob,
                timestamp: timestamp,
        ...
        // Store user's submission record
        const userPredRef = db.collection('userPredictions').doc(currentUser.username);
        ...
        showToast('✅ Forecasts submitted successfully!', 'success');
        ...
        showToast('Error submitting forecasts', 'error');
        submitBtn.disabled = false;
        submitBtn.textContent = 'Submit Forecasts';
    }
}
...
async function renderPredictionResults(weekKey) {
    ...
    container.innerHTML = '<div class="pred-loading">Loading results...</div>';
    ...
    try {
        ...
        // Fetch aggregated data for each question
        const resultsHTML = await Promise.all(PREDICTION_QUESTIONS.map(async (q, idx) => {
            const qRef = db.collection('predictions').doc(weekKey).collection(q.id);
            const qSnap = await qRef.get();
            
            const allProbs = [];
            qSnap.forEach(doc => {
                allProbs.push(doc.data().probability);
            });
            
            const userProb = userAnswers[idx];
            const mean = calculateMean(allProbs);
            const distribution = calculateDistribution(allProbs);
            ...
            return `
                <div class="pred-result-card">
                    ...
                    <div class="pred-result-stats">
                        <div class="pred-stat">
                            <span class="pred-stat-label">Your Forecast</span>
                            <span class="pred-stat-value user">${userProb}%</span>
                        </div>
                        <div class="pred-stat">
                            <span class="pred-stat-label">Crowd Mean</span>
                            <span class="pred-stat-value">${mean}%</span>
                        </div>
                        <div class="pred-stat">
                            <span class="pred-stat-label">Difference</span>
                            <span class="pred-stat-value ${diff >= 0 ? 'pos' : 'neg'}">${diff >= 0 ? '+' : ''}${diff}%</span>
                        </div>
                        <div class="pred-stat">
                            <span class="pred-stat-label">Participants</span>
                            <span class="pred-stat-value">${allProbs.length}</span>
                        </div>
                    </div>
        ...
        container.innerHTML = '<div class="pred-error">Error loading results</div>';
    }
}
...
async function renderPredictionChart(questionId, weekKey, userProb) {
    ...
    // Fetch data
    const qRef = db.collection('predictions').doc(weekKey).collection(questionId);
    const qSnap = await qRef.get();
    ...
    new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: Object.keys(distribution).map(bucket => 
                    bucket === userBucket ? 'rgba(0, 229, 255, 0.8)' : 'rgba(123, 146, 178, 0.5)'
                ),
                borderColor: Object.keys(distribution).map(bucket =>
                    bucket === userBucket ? '#00e5ff' : '#7b92b2'
                ),
                borderWidth: 2
            }]
        },
